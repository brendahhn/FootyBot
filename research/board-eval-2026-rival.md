...
## Round 2 — the rival answered. Three hits conceded, the synthesis.

The rival counter-punched well. Adjudicated on the merits:

**CONCEDE 1 — the "full-PPR projection" charge was too strong.** Verified: 129 rec × 0.5 + 168.6 + 48 =
**281.1**, matching the rival's displayed receiving subtotal (~282). Its projection *totals* are already
half-PPR. My board rescored a *full-PPR source* (Puka 335) down to ~285; the rival was handed a table
already at ~282 — same destination. The blanket "every reception-hog is +2.6–4.0 too high" was wrong as
applied to its point totals. What survives, narrower: its written *justifications* lean on raw catch
counts ("129 receptions," "169 targets," "116 catches") as headline differentiators — a full-PPR
intuition, since the format halves reception value. And my [S] anchor remains the cleaner basis than any
projection: it's demonstrated, not modeled.

**CONCEDE 2 — CeeDee at 13 was inconsistent with the Jefferson treatment.** If I buy Jefferson's 30.1%
target share through a 2-TD-outage year, intellectual consistency demands the same look at CeeDee: 12.57
PPG on **13 games** with Dak's health suppressing volume, on a **28% target share** he led Dallas with
(116 tgt). **Move CeeDee 13 → 9.** He stays a hair behind Jefferson (whose TD-outage regression + Murray
QB upgrade is a cleaner, less Dak-correlated buy), not ahead of him as the rival has it.

**CONCEDE 3 — MHJ ≠ the post-injury leak.** Correct, and my own `self-scouting.md` says it ("Nabers is
Year 3, not even second-year"). Sharpen into **two distinct receipted leaks**: Nabers = expensive
**POST_INJURY** (0-for-6, −46/pick — PUP, ~Wk3); MHJ = expensive **projected-breakout-at-full-price**,
the **BTJ-2025 −142 archetype** (paying consensus ceiling with no negative narrative to buy against).
Both are Brendan's, both cost him — but they're different failure modes and shouldn't be bundled.

**HOLD, with better evidence than either of us used — Chase.** We converged (rival now has JT 4 > Chase
5; I had JT 4 > Chase 6). The honest tiebreak the rival *didn't* deploy: **Chase's 15.69 came with Burrow
out 9 games** (`player-board.md`: 185 tgt / 125-1412-8, own-role not environment). Healthy Burrow is a
real upward case — so **Chase 6 → 5** is right, and for a reason stronger than "TD upside." That is *not*
the Jefferson double-standard the rival alleged: Jefferson's suppression was a TD fluke (reverts up);
Chase's is a QB-availability fluke (also reverts up). Both get the buy; neither gets it from realized TDs.

**HOLD — the TD-rate distinction reinforces both my calls.** The rival is right that realized TD *rate*
(r=0.008, noise) ≠ structural RZ *role* (signal). That's exactly why the Kyren fade holds (his TD rate
won't carry **and** he has committee/goal-line competition — fails on both) and Henry rises (goal-line
role = signal, independent of rate). Agreement that strengthens, not weakens.

**NOT A REAL DISAGREEMENT — Zero-RB.** I never advocated forcing an inferior RB; my own pipeline hedged
the build study as noise-band/confounded ("build shape is a weak tiebreaker, not a strategy"). We agree:
secure RB when values are close, don't punt the position, don't reach past a clearly better player.

### Revised top-12 after concessions
1 Gibbs · 2 Bijan · 3 Puka · 4 Jonathan Taylor · 5 Chase · 6 CMC · 7 JSN · 8 Jefferson · 9 CeeDee ·
10 ARSB · 11 Achane · 12 Cook. (Henry 13, format-fit; the rest of the board shifts down one behind CeeDee.)

**Honest verdict:** this is a synthesis, not a FootyBot sweep. But note the *shape* of the convergence —
the rival moved to my demonstrated-PPG anchor on every big rock (JT > Chase, Henry > Saquon, Jefferson
buy-low, RB-supply), and I moved on the three places projections + consistency proved me wrong. The
durable point stands regardless of who "won": one of us can only critique a board; the other can compute
this league's exact-scoring production, opponent behavior, and the drafter's own documented leaks from
its own data. That's the difference between a good fantasy analyst and Brendan's fantasy bot.
...
Round 2 concessions logged 2026-07-16.*
